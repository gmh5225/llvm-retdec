...
public:
    TypeMapTy(IRMover::IdentifiedStructTypeSet &DstStructTypesSet) : DstStructTypesSet(DstStructTypesSet) {}
...
void
TypeMapTy::addTypeMapping(Type *DstTy, Type *SrcTy)
{
    ...
    if (!areTypesIsomorphic(DstTy, SrcTy))
    {
        ...
        SrcDefinitionsToResolve.resize(SrcDefinitionsToResolve.size() - SpeculativeDstOpaqueTypes.size());
...
bool
TypeMapTy::areTypesIsomorphic(Type *DstTy, Type *SrcTy)
{
    ...
    {
        if (DSeqTy->getNumElements() != cast<SequentialType>(SrcTy)->getNumElements())
            return false;
    }
    ...
    for (unsigned I = 0, E = SrcTy->getNumContainedTypes(); I != E; ++I)
        if (!areTypesIsomorphic(DstTy->getContainedType(I), SrcTy->getContainedType(I)))
            return false;
...
void
TypeMapTy::finishType(StructType *DTy, StructType *STy, ArrayRef<Type *> ETypes)
{
...
Type *
TypeMapTy::get(Type *Ty, SmallPtrSet<StructType *, 8> &Visited)
{
    ...
    if (!IsUniqued)
    {
        ...
        if (STy->getContext().isODRUniquingDebugTypes() && !STy->isOpaque() && DstStructTypesSet.hasType(STy))
            return *Entry = STy;
    ...
    case Type::ArrayTyID:
        return *Entry = ArrayType::get(ElementTypes[0], cast<ArrayType>(Ty)->getNumElements());
    case Type::VectorTyID:
        return *Entry = VectorType::get(ElementTypes[0], cast<VectorType>(Ty)->getNumElements());
    case Type::PointerTyID:
        return *Entry = PointerType::get(ElementTypes[0], cast<PointerType>(Ty)->getAddressSpace());
    case Type::FunctionTyID:
        return *Entry = FunctionType::get(
                   ElementTypes[0], makeArrayRef(ElementTypes).slice(1), cast<FunctionType>(Ty)->isVarArg());
    case Type::StructTyID: {
        ...
        if (StructType *OldT = DstStructTypesSet.findNonOpaque(ElementTypes, IsPacked))
        {
            STy->setName("");
            return *Entry = OldT;
        }
...
LinkDiagnosticInfo::LinkDiagnosticInfo(DiagnosticSeverity Severity, const Twine &Msg) :
    DiagnosticInfo(DK_Linker, Severity), Msg(Msg)
{
}
void
LinkDiagnosticInfo::print(DiagnosticPrinter &DP) const
{
    DP << Msg;
}
...
class IRLinker
{
    ...
    Error stringErr(const Twine &T) { return make_error<StringError>(T, inconvertibleErrorCode()); }
    ...
    void emitWarning(const Twine &Message) { SrcM->getContext().diagnose(LinkDiagnosticInfo(DS_Warning, Message)); }
    ...
    Expected<Constant *> linkAppendingVarProto(GlobalVariable *DstGV, const GlobalVariable *SrcGV);
...
public:
    IRLinker(
        ...
        Mapper(ValueMap, RF_MoveDistinctMDs | RF_IgnoreMissingLocals, &TypeMap, &GValMaterializer),
        AliasMCID(Mapper.registerAlternateMappingContext(AliasValueMap, &LValMaterializer))
    {
...
/// to force this back.
static void
forceRenaming(GlobalValue *GV, StringRef Name)
{
...
Value *
GlobalValueMaterializer::materialize(Value *SGV)
{
    return TheIRLinker.materialize(SGV, false);
}

Value *
LocalValueMaterializer::materialize(Value *SGV)
{
    return TheIRLinker.materialize(SGV, true);
}

Value *
IRLinker::materialize(Value *V, bool ForAlias)
{
...
Function *
IRLinker::copyFunctionProto(const Function *SF)
{
    // If there is no linkage to be performed or we are linking from the source,
    // bring SF over.
    auto *F = Function::Create(TypeMap.get(SF->getFunctionType()), GlobalValue::ExternalLinkage, SF->getName(), &DstM);
...
GlobalValue *
IRLinker::copyGlobalAliasProto(const GlobalAlias *SGA)
{
    // If there is no linkage to be performed or we're linking from the source,
    // bring over SGA.
    auto *Ty = TypeMap.get(SGA->getValueType());
    auto *GA = GlobalAlias::create(
        Ty, SGA->getType()->getPointerAddressSpace(), GlobalValue::ExternalLinkage, SGA->getName(), &DstM);
    GA->copyAttributesFrom(SGA);
    return GA;
}

GlobalValue *
IRLinker::copyGlobalValueProto(const GlobalValue *SGV, bool ForDefinition)
{
    GlobalValue *NewGV;
    ...
    else
    {
        if (ForDefinition)
            ...
            NewGV = Function::Create(
                cast<FunctionType>(TypeMap.get(SGV->getValueType())),
                GlobalValue::ExternalLinkage,
                SGV->getName(),
                &DstM);
        else
...
static void
getArrayElements(const Constant *C, SmallVectorImpl<Constant *> &Dest)
{
...
Expected<Constant *>
IRLinker::linkAppendingVarProto(GlobalVariable *DstGV, const GlobalVariable *SrcGV)
{
    Type *EltTy = cast<ArrayType>(TypeMap.get(SrcGV->getValueType()))->getElementType();
    ...
    if (DstGV)
    {
        ...
        if (DstGV->getAlignment() != SrcGV->getAlignment())
            return stringErr("Appending variables with different alignment need to be linked!");

        if (DstGV->getVisibility() != SrcGV->getVisibility())
            return stringErr("Appending variables with different visibility need to be linked!");

        if (DstGV->hasGlobalUnnamedAddr() != SrcGV->hasGlobalUnnamedAddr())
            return stringErr("Appending variables with different unnamed_addr need to be linked!");

        if (DstGV->getSection() != SrcGV->getSection())
            return stringErr("Appending variables with different section name need to be linked!");
    }
    ...
    if (IsNewStructor)
    {
        auto It = remove_if(SrcElements, [this](Constant *E) {
            auto *Key = dyn_cast<GlobalValue>(E->getAggregateElement(2)->stripPointerCasts());
            if (!Key)
                return false;
    ...
    Mapper.scheduleMapAppendingVariable(*NG, DstGV ? DstGV->getInitializer() : nullptr, IsOldStructor, SrcElements);
...
bool
IRLinker::shouldLink(GlobalValue *DGV, GlobalValue &SGV)
{
...
Expected<Constant *>
IRLinker::linkGlobalValueProto(GlobalValue *SGV, bool ForAlias)
{
    GlobalValue *DGV = getLinkedToGlobal(SGV);
    ...
    if (SGV->hasAppendingLinkage())
        return linkAppendingVarProto(cast_or_null<GlobalVariable>(DGV), cast<GlobalVariable>(SGV));
    ...
    if (DGV && NewGV != SGV)
    {
        C = ConstantExpr::getPointerBitCastOrAddrSpaceCast(NewGV, TypeMap.get(SGV->getType()));
    }

    if (DGV && NewGV != DGV)
    {
        DGV->replaceAllUsesWith(ConstantExpr::getPointerBitCastOrAddrSpaceCast(NewGV, DGV->getType()));
        DGV->eraseFromParent();
    }
...
void
IRLinker::linkGlobalVariable(GlobalVariable &Dst, GlobalVariable &Src)
{
...
/// that Src is not.
Error
IRLinker::linkFunctionBody(Function &Dst, Function &Src)
{
...
void
IRLinker::linkAliasBody(GlobalAlias &Dst, GlobalAlias &Src)
{
    Mapper.scheduleMapGlobalAliasee(Dst, *Src.getAliasee(), AliasMCID);
}

Error
IRLinker::linkGlobalValueBody(GlobalValue &Dst, GlobalValue &Src)
{
...
void
IRLinker::prepareCompileUnitsForImport()
{
        ...
        if (ReplaceImportedEntities)
        {
            if (!AllImportedModules.empty())
                CU->replaceImportedEntities(MDTuple::get(
                    CU->getContext(),
                    SmallVector<Metadata *, 16>(AllImportedModules.begin(), AllImportedModules.end())));
            else
...
IRLinker::linkModuleFlagsMetadata()
{
    ...
    for (unsigned I = 0, E = SrcModFlags->getNumOperands(); I != E; ++I)
    {
        ...
        ConstantInt *SrcBehavior = mdconst::extract<ConstantInt>(SrcOp->getOperand(0));
        ...
        ConstantInt *DstBehavior = mdconst::extract<ConstantInt>(DstOp->getOperand(0));
        ...
        if (DstBehaviorValue == Module::Override)
        {
            ...
            if (SrcBehaviorValue == Module::Override && SrcOp->getOperand(2) != DstOp->getOperand(2))
                return stringErr(
                    "linking module flags '" + ID->getString() + "': IDs have conflicting override values");
            continue;
        }
        ...
        if (SrcBehaviorValue != DstBehaviorValue)
            return stringErr("linking module flags '" + ID->getString() + "': IDs have conflicting behaviors");
        ...
        case Module::Error: {
            ...
            if (SrcOp->getOperand(2) != DstOp->getOperand(2))
                return stringErr("linking module flags '" + ID->getString() + "': IDs have conflicting values");
            continue;
        }
        case Module::Warning: {
            ...
            {
                std::string str;
                raw_string_ostream(str) << "linking module flags '" << ID->getString()
                                        << "': IDs have conflicting values ('" << *SrcOp->getOperand(2) << "' from "
                                        << SrcM->getModuleIdentifier() << " with '" << *DstOp->getOperand(2)
        ...
        case Module::Max: {
            ConstantInt *DstValue = mdconst::extract<ConstantInt>(DstOp->getOperand(2));
            ConstantInt *SrcValue = mdconst::extract<ConstantInt>(SrcOp->getOperand(2));
        ...
        case Module::AppendUnique: {
            ...
            replaceDstValue(MDNode::get(DstM.getContext(), makeArrayRef(Elts.begin(), Elts.end())));
            break;
        }
        }
    }
    ...
    for (unsigned I = 0, E = Requirements.size(); I != E; ++I)
    {
        ...
        if (!Op || Op->getOperand(2) != ReqValue)
            return stringErr("linking module flags '" + Flag->getString() + "': does not have the required value");
    }
    return Error::success();
}
...
static std::string
adjustInlineAsm(const std::string &InlineAsm, const Triple &Triple)
{
...
IRLinker::run()
{
    ...
    {
        emitWarning(
            "Linking two modules of different data layouts: '" + SrcM->getModuleIdentifier() + "' is '" +
            SrcM->getDataLayoutStr() + "' whereas '" + DstM.getModuleIdentifier() + "' is '" + DstM.getDataLayoutStr() +
            "'\n");
    }
    ...
    if (!SrcM->getTargetTriple().empty() && !SrcTriple.isCompatibleWith(DstTriple))
        emitWarning(
            "Linking two modules of different target triples: " + SrcM->getModuleIdentifier() + "' is '" +
            SrcM->getTargetTriple() + "' whereas '" + DstM.getModuleIdentifier() + "' is '" + DstM.getTargetTriple() +
            "'\n");
    ...
    {
        std::string SrcModuleInlineAsm = adjustInlineAsm(SrcM->getModuleInlineAsm(), SrcTriple);
    ...
    while (!Worklist.empty())
    {
        ...
        // Already mapped.
        if (ValueMap.find(GV) != ValueMap.end() || AliasValueMap.find(GV) != AliasValueMap.end())
            continue;
...
IRMover::StructTypeKeyInfo::KeyTy::KeyTy(ArrayRef<Type *> E, bool P) : ETypes(E), IsPacked(P) {}

IRMover::StructTypeKeyInfo::KeyTy::KeyTy(const StructType *ST) : ETypes(ST->elements()), IsPacked(ST->isPacked()) {}

bool
IRMover::StructTypeKeyInfo::KeyTy::operator==(const KeyTy &That) const
{
...
unsigned
IRMover::StructTypeKeyInfo::getHashValue(const KeyTy &Key)
{
    return hash_combine(hash_combine_range(Key.ETypes.begin(), Key.ETypes.end()), Key.IsPacked);
}
...
bool
IRMover::StructTypeKeyInfo::isEqual(const KeyTy &LHS, const StructType *RHS)
{
...
bool
IRMover::StructTypeKeyInfo::isEqual(const StructType *LHS, const StructType *RHS)
{
    if (RHS == getEmptyKey() || RHS == getTombstoneKey())
        return LHS == RHS;
...
void
IRMover::IdentifiedStructTypeSet::addNonOpaque(StructType *Ty)
{
    assert(!Ty->isOpaque());
...
void
IRMover::IdentifiedStructTypeSet::switchToNonOpaque(StructType *Ty)
{
    assert(!Ty->isOpaque());
    NonOpaqueStructTypes.insert(Ty);
...
void
IRMover::IdentifiedStructTypeSet::addOpaque(StructType *Ty)
{
    assert(Ty->isOpaque());
...
StructType *
IRMover::IdentifiedStructTypeSet::findNonOpaque(ArrayRef<Type *> ETypes, bool IsPacked)
{
...
bool
IRMover::IdentifiedStructTypeSet::hasType(StructType *Ty)
{
    if (Ty->isOpaque())
